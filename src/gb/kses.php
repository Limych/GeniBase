 ...
 *
 * Added gb_ prefix to avoid conflicts with existing kses users
 *
 * @version 0.2.2
 ...
 *
 * The gb_kses_allowed_html filter is more powerful and supplies context.
 ...
 */
function gb_kses($string, $allowed_html, $allowed_protocols = array())
{
    if (empty($allowed_protocols))
        $allowed_protocols = gb_allowed_protocols();
    $string = gb_kses_no_null($string);
    $string = gb_kses_js_entities($string);
    $string = gb_kses_normalize_entities($string);
    $string = gb_kses_hook($string, $allowed_html, $allowed_protocols); // WP changed the order of these funcs and added args to gb_kses_hook
    return gb_kses_split($string, $allowed_html, $allowed_protocols);
}
...
function gb_kses_allowed_html($context = '')
{
    ...
    if (is_array($context)) {
         ...
         * @param string $context
         *            Context to judge allowed tags by. Allowed values are 'post',
 ...
 */
function gb_kses_hook($string, $allowed_html, $allowed_protocols)
{
    /**
 ...
 */
function gb_kses_split($string, $allowed_html, $allowed_protocols)
{
    global $pass_allowed_html, $pass_allowed_protocols;
    $pass_allowed_html = $allowed_html;
    $pass_allowed_protocols = $allowed_protocols;
    return preg_replace_callback('%(<!--.*?(-->|$))|(<[^>]*(>|$)|>)%', '_gb_kses_split_callback', $string);
}
...
{
    global $pass_allowed_html, $pass_allowed_protocols;
    return gb_kses_split2($match[0], $pass_allowed_html, $pass_allowed_protocols);
}

/**
 * Callback for gb_kses_split for fixing malformed HTML tags.
 *
 ...
 */
function gb_kses_split2($string, $allowed_html, $allowed_protocols)
{
    $string = gb_kses_stripslashes($string);
    ...
    if ('<!--' == substr($string, 0, 4)) {
        ...
        while ($string != ($newstring = gb_kses($string, $allowed_html, $allowed_protocols)))
            $string = $newstring;
        if ($string == '')
            return '';
    ...
    return gb_kses_attr($elem, $attrlist, $allowed_html, $allowed_protocols);
}

/**
 * Removes all attributes, if none are allowed for this element.
 *
 * If some are allowed it calls gb_kses_hair() to split them further, and then
...
function gb_kses_attr($element, $attr, $allowed_html, $allowed_protocols)
{
    ...
    if (! isset($allowed_html[strtolower($element)]) || count($allowed_html[strtolower($element)]) == 0)
        ...
        // Split it
    $attrarr = gb_kses_hair($attr, $allowed_protocols);
 ...
 */
function gb_kses_hair($attr, $allowed_protocols)
{
    $attrarr = array();
    $mode = 0;
    $attrname = '';
            ...
            case 2: // attribute value, a URL after href= for instance
                ...
                // "value"
                {
                    ...
                    if (in_array(strtolower($attrname), $uris))
                        $thisval = gb_kses_bad_protocol($thisval, $allowed_protocols);
 ...
 */
function gb_kses_check_attr_val($value, $vless, $checkname, $checkvalue)
{
    $ok = true;
 ...
 */
function gb_kses_bad_protocol($string, $allowed_protocols)
{
    $string = gb_kses_no_null($string);
    $iterations = 0;
    ...
    do {
        ...
        $string = gb_kses_bad_protocol_once($string, $allowed_protocols);
 ...
 * @return string
 */
function gb_kses_no_null($string)
{
 ...
 */
function gb_kses_stripslashes($string)
{
 ...
 */
function gb_kses_array_lc($inarray)
{
    $outarray = array();
 ...
 * @return string
 */
function gb_kses_js_entities($string)
{
    return preg_replace('%&\s*\{[^}]*(\}\s*;?|$)%', '', $string);
}

/**
 * Handles parsing errors in gb_kses_hair().
 *
 ...
 * @return string
 */
function gb_kses_html_error($string)
{
    return preg_replace('/^("[^"]*("|$)|\'[^\']*(\'|$)|\S)*\s*/', '', $string);
}
 ...
 */
function gb_kses_bad_protocol_once($string, $allowed_protocols, $count = 1)
{
    $string2 = preg_split('/:|&#0*58;|&#x0*3a;/i', $string, 2);
    if (isset($string2[1]) && ! preg_match('%/\?%', $string2[0])) {
        $string = trim($string2[1]);
        $protocol = gb_kses_bad_protocol_once2($string2[0], $allowed_protocols);
        if ('feed:' == $protocol) {
            if ($count > 2)
                return '';
            $string = gb_kses_bad_protocol_once($string, $allowed_protocols, ++ $count);
            if (empty($string))
                return $string;
        }
...
/**
 * Callback for gb_kses_bad_protocol_once() regular expression.
 *
 ...
 * @return string Sanitized content
 */
function gb_kses_bad_protocol_once2($string, $allowed_protocols)
{
    $string2 = gb_kses_decode_entities($string);
    $string2 = preg_replace('/\s/', '', $string2);
    $string2 = gb_kses_no_null($string2);
 ...
 */
function gb_kses_normalize_entities($string)
{
    ...
    $string = preg_replace_callback('/&amp;([A-Za-z]{2,8}[0-9]{0,2});/', 'gb_kses_named_entities', $string);
    $string = preg_replace_callback('/&amp;#(0*[0-9]{1,7});/', 'gb_kses_normalize_entities2', $string);
    $string = preg_replace_callback('/&amp;#[Xx](0*[0-9A-Fa-f]{1,6});/', 'gb_kses_normalize_entities3', $string);
    
    return $string;
}

/**
 * Callback for gb_kses_normalize_entities() regular expression.
 *
 ...
 */
function gb_kses_named_entities($matches)
{
 ...
 * Callback for gb_kses_normalize_entities() regular expression.
 *
 * This function helps {@see gb_kses_normalize_entities()} to only accept 16-bit
 ...
 * @return string Correctly encoded entity
 */
function gb_kses_normalize_entities2($matches)
{
    if (empty($matches[1]))
        return '';
...
/**
 * Callback for gb_kses_normalize_entities() for regular expression.
 *
 * This function helps gb_kses_normalize_entities() to only accept valid Unicode
 ...
 * @return string Correctly encoded entity
 */
function gb_kses_normalize_entities3($matches)
{
    if (empty($matches[1]))
        return '';
 ...
 */
function gb_kses_decode_entities($string)
{
    $string = preg_replace_callback('/&#([0-9]+);/', '_gb_kses_decode_entities_chr', $string);
    $string = preg_replace_callback('/&#[Xx]([0-9A-Fa-f]+);/', '_gb_kses_decode_entities_chr_hexdec', $string);
    
    return $string;
}

/**
 * Regex callback for gb_kses_decode_entities()
 *
 * @param array $match
 *            preg match
 * @return string
 */
function _gb_kses_decode_entities_chr($match)
{
    return chr($match[1]);
}
 ...
 * @return string
 */
function _gb_kses_decode_entities_chr_hexdec($match)
{
 ...
 */
function gb_filter_post_kses($data)
{
    return addslashes(gb_kses(stripslashes($data), 'post'));
}
 ...
 * @return string Filtered post content with allowed HTML tags and attributes intact.
 */
function gb_kses_post($data)
{
    return gb_kses($data, 'post');
}
 ...
 */
function gb_filter_nohtml_kses($data)
{
    return addslashes(gb_kses(stripslashes($data), 'strip'));
}

/**
 * Adds all Kses input form content filters.
 *
 * All hooks have default priority. The gb_filter_kses() function is added to
...
function safecss_filter_attr($css, $deprecated = '')
{
     ...
     */
    $allowed_attr = GB_Hooks::apply_filters('safe_style_css', array(
        'text-align',
        'margin',
        'color',
        'float',
 ...
 */
function _gb_add_global_attributes($value)
{
