 ...
 *        
 * @return GB_Styles GB_Styles object for printing styles.
 */
function gb_styles()
{
...
function gb_print_styles($handles = false)
{
    ...
    return gb_styles()->do_items($handles);
}
...
function gb_add_inline_style($handle, $data)
{
    ...
    return gb_styles()->add_inline_style($handle, $data);
}
 ...
 *            Path to the stylesheet from the GeniBase root directory. Example: '/css/mystyle.css'.
 * @param array $deps
...
{
    gb_styles()->add($handle, $src, $deps, $ver, $media);
}
 ...
 *            Name of the stylesheet.
 * @param string|bool $src
 *            Path to the stylesheet from the root directory of GeniBase. Example: '/css/mystyle.css'.
 * @param array $deps
 *            An array of registered style handles this stylesheet depends on. Default empty array.
 * @param string|bool $ver
...
function gb_enqueue_style($handle, $src = false, $deps = array(), $ver = false, $media = 'all')
{
    if ($src) {
        ...
        gb_styles()->add($_handle[0], $src, $deps, $ver, $media);
    }
    gb_styles()->enqueue($handle);
}
...
{
    return (bool) gb_styles()->query($handle, $list);
}
...
{
    return gb_styles()->add_data($handle, $key, $value);
}
