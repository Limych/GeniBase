 ...
 * @param array $names
 *            of names.
 * @return array array of arrays of metakeys. All keys are upcased.
 */
 ...
 *            или массив слов.
 * @param boolean $trim_surname
 *            передана фамилия. У фамилий сокращаются типичные окончания.
 * @return string|string[] ключ слова или набор ключей для набора слов.
 */
function rus_metaphone($word, $trim_surname = false)
{
    ...
    static $ends = array( // Шаблоны для «сжатия» окончания наиболее распространённых фамилий
        ...
        '/[ЕИ]?Н(?:ОК|КО(?:В|ВА)?)$/uS' => '2', // -енко, -енков, -енкова, -енок, -инко, -инков, -инкова, -инок, -нко, -нков, -нкова, -нок
...
;
    ...
    if ($trim_surname)
        $word = preg_replace(array_keys($ends), array_values($ends), $word);
 ...
 *            array of names.
 * @return array array of names and metakeys.
 *        
 */
function gb_metaphone($metakeys)
{
    ...
    foreach ($names as $nk) {
        if (! empty($metakeys[$nk]))
            $metakeys[$nk + GB_MK_KEY_PHONE] = rus_metaphone($metakeys[$nk], ($nk === GB_MK_SURNAME));
    }
    return $metakeys;
}
if (class_exists('GB_Hooks'))
 ...
 * @param string $text
 *            text.
 * @return string|string[] text or translation table if $text was set to NULL.
 */
