...
function gb_load_translations_early()
{
    ...
    while (true) {
        if (defined('GB_LANG')) {
            if ('' == GB_LANG)
                break;
            $locales = preg_split('/[^\w\-]+/si', GB_LANG, - 1, PREG_SPLIT_NO_EMPTY);
        }
...
<html xmlns="http://www.w3.org/1999/xhtml"
	<?php print " lang='$lang' xml:lang='$lang'$rtl"; ?>>
<head>
...
<?php if( substr($lang, 0, 2) != 'en'): ?>
	<div lang="en" xml:lang="en" dir="ltr">
		<h1>Maintenance</h1>
		<p>This website is currently under going maintenance and will be back
			online shortly. Thank you for your patience.</p>
	</div>
<?php endif; ?>
</body>
</html>
 ...
 *            be displayed
 * @param mixed $ignore
 *            or backtrace levels count to be ignored in report
 */
function gb_debug_info($var, $ignore = 0)
{
    ...
    if ($ignore !== 0) {
        if (! is_string($ignore))
            ...
            $level = 1;
            while (isset($trace[$level]) && isset($trace[$level]['class']) && $trace[$level]['class'] == $ignore)
                $level ++;
            $level --;
        }
        
        $place = substr($trace[$level]['file'], strlen(BASE_DIR)) . ' #' . $trace[$level]['line'] . ' (' . $place . ')';
    }
...
function gb_start_object_cache()
{
    ...
    if (function_exists('gb_cache_add_global_groups'))
        gb_cache_add_global_groups(array(
            'users',
            'user-logins',
            'user-meta',
            'site-transient',
    ...
    if (function_exists('gb_cache_add_non_persistent_groups'))
        gb_cache_add_non_persistent_groups(array(
            'counts',
            'plugins'
        ));
}
