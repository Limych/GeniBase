 ...
 *            Optional. A number of stack frames to skip - useful for unwinding
 *            back to the source of the issue. Default 0.
 * @param bool $pretty
 *            Optional. Whether or not you want a comma separated string or raw
 ...
 *            Optional. Error title. If `$message` is a `GB_Error` object,
 ...
 *            When false uses the $_SERVER value. Default false.
 * @return string New URL query string.
 */
 ...
 * @param string $prefix
 *            Optional. Numeric index. If set, start parameter numbering with it.
 *            Default null.
 * @param string $sep
 *            Optional. Argument separator; defaults to 'arg_separator.output'.
 *            Default null.
 * @param string $key
 ...
 *            Optional Whether to force SSL login. Default null.
 ...
 * @param string $version
 *            The version of GeniBase that deprecated the file.
 * @param string $replacement
 *            Optional. The file that should have been included based on BASE_DIR.
 *            Default null.
 * @param string $message
...
function _deprecated_file($file, $version, $replacement = null, $message = '')
{
    ...
    if (class_exists('GB_Hooks')) {
         ...
         * @param string $replacement
         *            The file that should have been included based on BASE_DIR.
         * @param string $version
         *            The version of GeniBase that deprecated the file.
         * @param string $message
 ...
 */
function _gb_json_sanity_check($data, $depth)
{
    if ($depth < 0) {
    ...
    if (is_array($data)) {
        ...
        foreach ($data as $id => $el) {
            ...
            if (is_string($id)) {
                $clean_id = _gb_json_convert_string($id);
            } else {
                $clean_id = $id;
            }
            ...
            if (is_array($el) || is_object($el)) {
                ...
                $output[$clean_id] = _gb_json_convert_string($el);
            } else {
                $output[$clean_id] = $el;
            }
        }
        ...
        return _gb_json_convert_string($data);
    } else {
        return $data;
    }
 ...
 */
function _gb_json_convert_string($string)
{
    static $use_mb = null;
    if (is_null($use_mb)) {
    ...
    if ($use_mb) {
        ...
        return gb_check_invalid_utf8($string, true);
    }
}
 ...
 * @param int $depth
 *            Optional. Maximum depth to walk through $data. Must be
 *            greater than 0. Default 512.
 * @return bool|string The JSON encoded string, or false if it cannot be encoded.
 */
function gb_json_encode($data, $options = 0, $depth = 512)
{
    /*
 ...
 */
function gb_nonce_url($actionurl, $action = -1, $name = '_gbnonce')
{
    $actionurl = str_replace('&amp;', '&', $actionurl);
    return esc_html(add_query_arg($name, gb_create_nonce($action), $actionurl));
}
