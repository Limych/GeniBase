...
class GB_Transcriptor
{
     ...
     *            language code. Default current locale language.
     * @param int $mode
     *            mode. {@link GB_Transcriptor} Default self::MODE_TRANSCRIBE
     * @return string text.
     */
    static function transcript($text, $from_lang, $to_lang = null, $mode = self::MODE_TRANSCRIBE)
    {
        ...
        if (! isset(self::$trans_table) || ! is_array(self::$trans_table) || ! isset(self::$trans_table[$mode]) || ! is_array(self::$trans_table[$mode]))
            return $text;
     ...
     *            language code.
     * @param int $mode
     *            mode. {@link GB_Transcriptor}
     * @param array $tr
     *            translation table. {@link strtr}
     * @param array $tr_special
     *            translation table for special ocassions.
     *            {@link preg_replace}
     * @return boolean true.
     */
     ...
     * @param int $mode
     *            mode. {@link GB_Transcriptor} Default self::MODE_TRANSCRIBE
     * @return boolean if transcription table have. False otherwise.
     */
...
function gb_transcriptor_init()
{
    ...
    $tr_special = array(
        '/(?<=\b|[аоэиуыеёюяъь])е/uS' => 'jo',
        '/(?<=[жцчшщ])е/uS' => 'o',
        '/(?<=\b|[аоэиуыеёюяъь])ё/uS' => 'je',
        '/(?<=[жцчшщ])ё/uS' => 'e',
        '/(?<=[ь])и/uS' => 'ji',
        '/(?<=[жцш])и/uS' => 'y',
        '/л(?=[иья])/uS' => 'l',
...
if (class_exists('GB_Hooks'))
    GB_Hooks::add_action('init', 'gb_transcriptor_init');
else
    gb_transcriptor_init();
