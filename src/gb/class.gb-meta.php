...
class GB_Meta
{
     ...
     *        
     * @param string $type
     *            Type of object to get metadata table for (e.g., comment, post, or user)
     ...
     * @param bool $unique
     *            Optional, default is false. Whether the specified metadata key should be
    ...
    {
        // TODO: Remove this?
        // echo "<hr/><pre>\n".var_export(compact('meta_type', 'object_id', 'meta_key', 'meta_value'), true)."</pre><hr/>";
        // die;
         ...
         * @param bool $unique
         *            Whether the specified meta key should be unique
        ...
        if ($unique && gbdb()->get_cell('SELECT COUNT(*) FROM ?@table WHERE meta_key = ?key AND ?#column = ?id', array(
            '@table' => $table,
            'key' => $meta_key,
            '#column' => $column,
            'id' => $object_id
     ...
     *            ID of the object metadata is for
     * @param string $meta_key
     *            Optional. Metadata key. If not specified, retrieve all metadata for
     *            the specified object.
     * @param bool $single
     *            Optional, default is false. If true, return only the first value of the
    ...
    static function get($meta_type, $object_id, $meta_key = '', $single = false)
    {
        ...
        if (! $meta_cache) {
            $meta_cache = GB_Meta::update_cache($meta_type, array(
                $object_id
            ));
    ...
    static function update($meta_type, $object_id, $meta_key, $meta_value, $prev_value = '')
    {
        ...
        $meta_ids = gbdb()->get_column("SELECT `mID` FROM $table WHERE meta_key = ?meta_key AND ?#column = ?object_id", array(
            'meta_key' => $meta_key,
            '#column' => $column,
            'object_id' => $object_id
        ));
     ...
     *            Metadata key
     * @param mixed $meta_value
     *            Optional. Metadata value. Must be serializable if non-scalar. If specified,
     *            only delete metadata entries with this value. Otherwise, delete all entries with the specified
     *            meta_key.
     * @param bool $delete_all
     *            Optional, default is false. If true, delete matching metadata entries
    ...
    static function delete($meta_type, $object_id, $meta_key, $meta_value = '', $delete_all = false)
    {
         ...
         *            Meta value. Must be serializable if non-scalar.
         * @param bool $delete_all
         *            Whether to delete the matching metadata entries
        ...
        $query = gbdb()->prepare_query($query, array(
        ...
        if ($delete_all)
            $object_ids = gbdb()->get_column("SELECT ?#type_column FROM $table WHERE meta_key = ?meta_key", array(
                '#type_column' => $type_column,
                'meta_key' => $meta_key
            ));
         ...
         *       
         * @param array $meta_ids
         *            An array of deleted metadata entry IDs.
         * @param int $object_id
         *            Object ID.
         * @param string $meta_key
         *            Meta key.
    ...
    static function delete_by_mid($meta_type, $meta_id)
    {
        ...
        if ($meta = self::get_by_mid($meta_type, $meta_id)) {
            ...
            $result = (bool) gbdb()->delete($table, array(
                'mID' => $meta_id
            ));
    ...
    static function update_cache($meta_type, $object_ids)
    {
        ...
        if (empty($ids))
            ...
            // Get meta info
        $id_list = join(',', $ids);
        $meta_list = gbdb()->get_table("SELECT ?#column, meta_key, meta_value FROM $table WHERE ?#column IN (?id_list) ORDER BY mID ASC", array(
            '#column' => $column,
            'id_list' => $id_list
        ));
     ...
     * @param string $meta_key
     *            Meta key
     * @param string|array $sanitize_callback
     *            A function or method to call when sanitizing the value of $meta_key.
     * @param string|array $auth_callback
     *            Optional. A function or method to call when performing edit_*_meta, add_*_meta, and delete_*_meta capability checks.
     */
