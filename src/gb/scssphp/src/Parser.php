...
class Parser
{
    ...
    protected function stripAssignmentFlag(&$value)
    {
        ...
        for ($token = &$value; $token[0] === Type::T_LIST && ($s = count($token[2])); $token = &$lastNode) {
            ...
            if ($lastNode[0] === Type::T_KEYWORD && in_array($lastNode[1], [
                '!default',
                '!global'
            ])) {
                array_pop($token[2]);
    ...
    private function getSourcePosition($pos)
    {
        ...
        while ($low < $high) {
            ...
            return [
                $mid + 1,
                $pos - $this->sourcePositions[$mid]
            ];
        }
        
        return [
            $low + 1,
            $pos - $this->sourcePositions[$low]
        ];
    }
