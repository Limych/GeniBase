...
class Compiler
{
    ...
    protected function compileDirective(Block $block)
    {
        ...
        if ($block->name === 'keyframes' || substr($block->name, - 10) === '-keyframes') {
            $this->compileKeyframeBlock($block, [
                $s
            ]);
        } else {
            $this->compileNestedBlock($block, [
                $s
            ]);
        }
    }
    ...
    private function compileWith($with)
    {
        ...
        if ($this->libMapHasKey([
            $with,
            self::$with
        ])) {
            $without = self::WITH_ALL;
            
            $list = $this->coerceList($this->libMapGet([
                $with,
                self::$with
            ]));
            ...
            foreach ($query as $q) {
                switch ($q[0]) {
                    case Type::T_MEDIA_TYPE:
                        if ($type) {
                            $type = $this->mergeMediaTypes($type, array_map([
                                $this,
                                'compileValue'
                            ], array_slice($q, 1)));
        ...
        switch ($type) {
            case Type::T_EXPRESSION:
                ...
                if (is_callable([
                    $this,
                    $fn
                ]) || (($fn = "op${ucLType}${ucRType}") && is_callable([
                    $this,
                    $fn
                ]) && $passOp = true) || (($fn = "op${ucOpName}") && is_callable([
                    $this,
                    $fn
                ]) && $genOp = true)) {
                    $coerceUnit = false;
    ...
    protected function multiplyMedia(Environment $env = null, $childQueries = null)
    {
        ...
        $parentQueries = isset($env->block->queryList) ? $env->block->queryList : [
            [
                [
                    Type::T_MEDIA_VALUE,
                    $env->block->value
    ...
    protected function libMapMerge($args)
    {
        ...
        return [
            Type::T_MAP,
            array_merge($map1[1], $map2[1]),
            array_merge($map1[2], $map2[2])
        ];
    }
    ...
    protected function libStrInsert($args)
    {
        ...
        $string[2] = [
            substr_replace($stringContent, $insertContent, $index - 1, 0)
        ];
