 ...
 * @since	3.0.0
 */
abstract class GB_Session_Tokens
{
     ...
     * the session manager for a subclass of GB_Session_Tokens.
     *
     * @since 3.0.0
     * @access public
     * @static
     ...
     * @param int $user_id
     *            User whose session to manage.
     * @return GB_Session_Tokens of child of class
     */
    final public static function get_instance($user_id)
    {
         ...
         *            Default 'GB_User_Meta_Session_Tokens'.
         */
        $manager = GB_Hooks::apply_filters('session_token_manager', 'GB_User_Meta_Session_Tokens');
    ...
    final public function create($expiration)
    {
         ...
         *            User ID.
         */
        $session = GB_Hooks::apply_filters('attach_session_information', array(), $this->user_id);
        ...
        if (! empty($_SERVER['HTTP_USER_AGENT'])) {
            $session['ua'] = gb_unslash($_SERVER['HTTP_USER_AGENT']);
        }
        
        // Timestamp
        $session['login'] = time();
        
        $token = gb_generate_password(43, false, false);
    ...
    {
        /**
         * This filter is documented in gb/class.gb-session-tokens.php
         */
        $manager = GB_Hooks::apply_filters('session_token_manager', 'GB_User_Meta_Session_Tokens');
 ...
 * @since 3.0.0
 */
class GB_User_Meta_Session_Tokens extends GB_Session_Tokens
{
    ...
    {
        $sessions = GB_User::get_meta($this->user_id, 'session_tokens', true);
        
        if (! is_array($sessions)) {
            return array();
        }
        
        $sessions = array_map(array(
            $this,
            'prepare_session'
        ), $sessions);
        return array_filter($sessions, array(
            $this,
            'is_still_valid'
        ));
    }
    ...
    {
        if ($sessions) {
            GB_User::update_meta($this->user_id, 'session_tokens', $sessions);
        } else {
            GB_User::delete_meta($this->user_id, 'session_tokens');
        }
    }
    ...
    public static function drop_sessions()
    {
        Meta::delete('user', false, 'session_tokens', false, true);
    }
}
