...
class GB_Options
{
     ...
     *            Options section name. As a rule it's similar to plugin's name. Optional.
     ...
     *        
     * @param string $section
     *            name. As a rule it's similar to plugin's name.
     * @param string $option
     *            option to add. Expected to not be SQL-escaped.
     * @param mixed $value
     ...
     * @param string $section
     *            name. As a rule it's similar to plugin's name. Optional.
     * @param bool|mixed $autoload
     *            is enabled. Whether to load the option when GeniBase starts up.
     * @return bool if option was not added and true if option was added.
     */
     ...
     *       
     * @param string $option
     *            option to retrieve. Expected to not be SQL-escaped.
     * @param string $section
     *            name. As a rule it's similar to plugin's name. Optional.
     * @param mixed $default
     *            value to return if the option does not exist.
     * @return mixed set for the option.
     */
    static function get($option, $section = '', $default = false)
    {
        ...
        if (defined('GB_INSTALLING')) {
            $suppress = gbdb()->suppress_errors();
            $row = gbdb()->get_row('SELECT option_value FROM ?_options ' . 'WHERE section = ?section AND option_name = ?key LIMIT 1', array(
                'section' => $section,
                'key' => $option
            ));
            gbdb()->suppress_errors($suppress);
     ...
     * @param string $section
     *            name. As a rule it's similar to plugin's name. Optional.
     * @return bool if value was not updated and true if value was updated.
     */
    static function update($option, $value, $section = '')
    {
        ...
        if (self::$has_hooks) {
             ...
             *       
             * @param string $option_hash
             *            unique hash of option to update.
             * @param mixed $old_value
             *            old option value.
             * @param mixed $value
             *            new option value.
        ...
        $result = gbdb()->set_row('?_options', array(
            ...
            'section',
            'option_name'
        ), GB_DBase::MODE_UPDATE);
        // TODO: Remove this?
        // if($option == 'user_roles'){ echo "<hr/><pre>\n".print_r(compact('section', 'option', 'value', 'serialized_value'), true)."</pre><hr/>"; die; }
        if (! $result)
            return false;
        ...
        if (self::$has_hooks) {
             ...
             *       
             * @param string $option_hash
             *            unique hash of the updated option.
             * @param mixed $old_value
             *            old option value.
             * @param mixed $value
             *            new option value.
     ...
     * Prevents removal of protected GeniBase options.
     *
     * @since 2.3.0
     *       
     * @param string $option
    ...
    static function delete($option, $section = '')
    {
        ...
        $row = gbdb()->get_cell('SELECT autoload FROM ?_options WHERE section = ?section AND option_name = ?key', array(
            'section' => $section,
            'key' => $option
        ));
        if (! $row)
     ...
     *            Transient name. Expected to not be SQL-escaped. Must be
     *            45 characters or fewer in length.
     * @param mixed $value
     *            Transient value. Must be serializable if non-scalar.
     *            Expected to not be SQL-escaped.
     * @param int $expiration
     *            Optional. Time until expiration in seconds. Default 0.
