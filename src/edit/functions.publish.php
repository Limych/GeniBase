...
function prepublish_date(&$raw, &$date_norm)
{
    ...
    } elseif (preg_match("/^$reg_date_left$/uS", $date_norm, $matches) || preg_match("/^$reg_date_right$/uS", $date_norm, $matches)) {
        if (empty($matches[3]))
            $matches[3] = ($matches[2] >= 8 ? 1914 : 1915);
        $matches[4] = $matches[1];
        $matches[5] = $matches[2];
        $matches[6] = $matches[3];
        
        // Периодическая запись дд-дд.мм.гггг или дд.мм-дд.мм.гггг или дд.мм.гггг-дд.мм.гггг
    } elseif (preg_match("/^$reg_date_left-$reg_date_left$/uS", $date_norm, $matches) || preg_match("/^$reg_date_left-$reg_date_right$/uS", $date_norm, $matches) || preg_match("/^$reg_date_right-$reg_date_right$/uS", $date_norm, $matches)) {
        if (empty($matches[6]))
            $matches[6] = ($matches[5] >= 8 ? 1914 : 1915);
        if (empty($matches[2]))
            $matches[2] = $matches[5];
        if (empty($matches[3]))
            $matches[3] = $matches[6];
    ...
    } elseif (preg_match("/^(\d\d?),(?:\d\d?,)*$reg_date_left$/uS", $date_norm, $matches) || preg_match("/^(\d\d?),(?:\d\d?,)*$reg_date_right$/uS", $date_norm, $matches)) {
        if (empty($matches[4]))
            $matches[4] = ($matches[3] >= 8 ? 1914 : 1915);
        array_splice($matches, 2, 0, array(
            $matches[3],
            $matches[4]
        ));
    }
    ...
    if (! empty($matches)) {
        ...
        if ($matches[3] < 100)
            $matches[3] += 1900;
        if (empty($matches[2]))
            $matches[2] = ($matches[3] == 1914 ? '08' : '01');
        elseif (! is_numeric($matches[2]))
            $matches[2] = $month_names[$matches[2]];
        if (empty($matches[1]))
            $matches[1] = '01';
        $tmp = date_create(implode('-', array(
            $matches[3],
            $matches[2],
            $matches[1]
        )));
        ...
        if ($matches[6] < 100)
            $matches[6] += 1900;
        if (empty($matches[5]))
            $matches[5] = ($matches[5] == 1918 ? '11' : '12');
        elseif (! is_numeric($matches[5]))
            $matches[5] = $month_names[$matches[5]];
        if (empty($matches[4]))
            $matches[4] = (($matches[6] == 1918) && ($matches[5] == 11) ? '11' : $last_days[intval($matches[5]) - 1]);
        $tmp = date_create(implode('-', array(
            $matches[6],
            $matches[5],
            $matches[4]
        ...
        if (($raw['date_from'] < MIN_DATE) || ($raw['date_from'] > MAX_DATE) || ($raw['date_to'] < MIN_DATE) || ($raw['date_to'] > MAX_DATE)) {
            unset($raw['date_from']);
            unset($raw['date_to']);
        }
    }
...
function prepublish($raw, &$have_trouble, &$date_norm)
{
    ...
    if (isset($raw['religion'])) {
        ...
        if (empty($religion_conts)) {
            ...
            $result = gbdb()->get_table('SELECT `id`, `religion`, `contractions` FROM ?_dic_religions' . ' WHERE `religion` NOT LIKE "(%"');
    ...
    if (isset($raw['reason'])) {
        ...
        if (empty($reason_conts)) {
            $result = gbdb()->get_table('SELECT `reason_id`, `reason_raw` FROM ?_dic_reason2id AS rid,' . ' ?_dic_reasons AS r WHERE rid.reason_id = r.id and r.`reason` NOT LIKE "(%"');
    ...
    if (! empty($raw['uyezd'])) {
        $res = gbdb()->get_cell('SELECT id FROM ?_dic_regions WHERE parent_id = ?parent_id AND title LIKE ?title', array(
 ...
 *            для нормализации.
 * @param bool $have_trouble
 *            автоматически формализовать данные.
 * @return mixed[] версия данных.
 */
function prepublish_make_data($raw_norm, &$have_trouble)
{
    ...
    foreach (explode(' ', 'id surname name rank religion_id marital_id region_id place reason_id date date_from date_to source_id source_pg') as $key) {
        if (! isset($raw_norm[$key]) || ((empty($raw_norm[$key]) || absint($raw_norm[$key]) != $raw_norm[$key]) && $key != 'source_id' && preg_match('/(^|_)id$/uS', $key)) || ($key == 'date_from' && ($raw_norm['date_from'] < MIN_DATE || $raw_norm['date_from'] > MAX_DATE)) || ($key == 'date_to' && ($raw_norm['date_to'] < MIN_DATE || $raw_norm['date_to'] > MAX_DATE)))
            $have_trouble = true;
        else
