...
if (isset($_POST['reg_from'])) {
    ...
    if ($reg_from && $reg_to) {
        if (!GB_DEBUG) {    // В режиме отладки реальных изменений в базе не производим
            ...
            gbdb()->set_row('?_dic_regions', array('region_ids' => ''), array('id' => $reg_from['id']));
        }
...
?>
    <form method="post" class="editor">
        <p>Удалить регион с ID <input type="number" name="reg_from" min="1" max="<?php print $max_id; ?>"/>, а все
            записи из него перенести в регион <input type="number" name="reg_to" min="1" max="<?php print $max_id; ?>"/>.
        </p>
