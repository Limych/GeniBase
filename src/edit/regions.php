...
{
    $result = gbdb()->get_table('SELECT id, title, region_comment FROM ?_dic_regions WHERE parent_id = ?id' . ' ORDER BY title', array(
        'id' => $parent_id
    ));
    foreach ($result as $row) {
        print "\t<option value='" . $row['id'] . "'>" . esc_html($row['title']) . (empty($row['region_comment']) ? '' : ' <span class="comment">' . esc_html($row['region_comment']) . '</span>') . "</option>\n";
