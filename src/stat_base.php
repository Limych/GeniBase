...
SELECT
 ...
 ,SUM(CASE WHEN (INSTR(url,'list_nr='       ) >0) THEN 1 ELSE 0 END) `Поиск по № списка` 
...
while($row = $result->fetch_array(MYSQLI_NUM)){
	...
	print "<tr class='" . ($even ? 'even' : 'odd') . "'>\n\t<td>" 
 		               ...
 		               // . "<td class='alignright'>" . format_num($row[0]) . "</td>\n"
 						...
 						. "<td class='alignright'>" . format_num($row[4]) . "</td>\n"
 						. "<td class='alignright'>" . format_num($row[5]) . "</td>\n"
				 		. "<td class='alignright'>" . format_num($row[6]) . "</td>\n"
	            		. "<td class='alignright'>" . format_num($row[7]) . "</td>\n"
            			. "<td class='alignright'>" . format_num($row[8]) . "</td>\n"
						. "<td class='alignright'>" . format_num($row[9]) . "</td>\n"
						. "<td class='alignright'>" . format_num($row[10]). "</td>\n"
 		                . "<td class='alignright'>" . format_num($row[11]). "</td>\n"
 		                . "<td class='alignright'>" . format_num($row[12]). "</td>\n"
 						. "<td class='alignright'>" . format_num($row[13]). "</td>\n"
 		                . "</tr>";
}
$result->free();
?>
