...
class ww1_solders_set extends ww1_records_set
{
    ...
    function __construct($page, $data = array(), $records_cnt = NULL)
    {
        ...
        foreach ($data as $row) {
            if ($row['religion'] == '(иное)')
                $row['religion'] = gbdb()->get_cell('SELECT religion FROM ?_persons_raw WHERE `id` = ?id', array(
                    'id' => $row['id']
                ));
            
            if ($row['marital'] == '(иное)')
                $row['marital'] = gbdb()->get_cell('SELECT marital FROM ?_persons_raw WHERE `id` = ?id', array(
                    'id' => $row['id']
                ));
            
            if ($row['reason'] == '(иное)')
                $row['reason'] = gbdb()->get_cell('SELECT reason FROM ?_persons_raw WHERE `id` = ?id', array(
                    'id' => $row['id']
                ));
    ...
    private function _show_report($brief_fields, $detailed_fields)
    {
        ...
        if (false !== ($show_detailed = ! empty($detailed_fields))) {
            gb_enqueue_script('ww1-records', site_url('/js/ww1-records.js'), array(
                'jquery'
            ));
        }
        ...
        foreach ($this->records as $row) {
            ...
            $details = '. ' . ($row['surname'] ? $row['surname'] : '<span class="na">' . __('(surname is not&nbsp;specified)', WW1_TXTDOM) . '</span>') . ', ' . ($row['name'] ? $row['name'] : '<span class="na">' . __('(name is not&nbsp;specified)', WW1_TXTDOM) . '</span>');
...
<?php
    ...
    {
        if ($this->records_cnt) {
            print '<p class="align-center">' . sprintf(_n('Found %s record.', 'Found %s records.', $this->records_cnt, WW1_TXTDOM), number_format_i18n($this->records_cnt)) . '</p>';
            $this->_show_report($brief_fields, $detailed_fields);
        } else {
            print '<p class="align-center">' . __('Unfortunately, nothing found.', WW1_TXTDOM) . '</p>';
            ?>
<div class="notfound">
			<p><?php _e('What to do now?', WW1_TXTDOM); ?></p>
			<ol>
