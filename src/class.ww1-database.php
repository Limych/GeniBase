...
if (! defined('GB_DEBUG_SQL_PROF'))
    define('GB_DEBUG_SQL_PROF', false);
...
if (! defined('MIN_DATE'))
    define('MIN_DATE', '1914-07-28');
if (! defined('MAX_DATE'))
    define('MAX_DATE', '1918-11-11');
    ...
    function search_form()
    {
        ...
        if ($this->query_mode == Q_SIMPLE) {
            ...
            foreach ($fields as $key => $val) {
                print "<div class='field'>" . "<label for='q_$key'>$val:</label>" . "<div><input type='text' id='q_$key' name='$key' value='" . esc_attr($this->query[$key]) . "' /></div>" . "</div>\n";
            }
        } else {
            // Расширенный режим поиска **************************************
            $dics = array();
            ...
            $dics['rank'] = gbdb()->get_column('SELECT DISTINCT rank, rank FROM ?_persons WHERE rank != ""' . ' ORDER BY rank', array(), TRUE);
            
            // Получаем список всех вариантов значений вероисповеданий
            $dics['religion'] = gbdb()->get_column('SELECT id, religion FROM ?_dic_religions' . ' WHERE religion_cnt != 0 ORDER BY religion', array(), TRUE);
            
            // Получаем список всех вариантов значений семейных положений
            $dics['marital'] = gbdb()->get_column('SELECT id, marital FROM ?_dic_maritals' . ' WHERE marital_cnt != 0 ORDER BY marital', array(), TRUE);
            
            // Получаем список всех вариантов значений событий
            $dics['reason'] = gbdb()->get_column('SELECT id, reason FROM ?_dic_reasons WHERE reason_cnt != 0' . ' ORDER BY reason', array(), TRUE);
            
            // Получаем список всех вариантов значений типов источников
            $dics['source_type'] = gbdb()->get_column('SELECT id, source_type FROM ?_dic_source_types WHERE source_type_cnt != 0' . ' ORDER BY source_type', array(), TRUE);
            ...
            foreach ($fields as $key => $val) {
                switch ($key) {
                    case 'surname':
                        // Текстовые поля
                        print "<div class='fieldset'>" . "<label for='q_$key'>$val:</label>" . "<div>" . "<div class='field'><input type='text' id='q_$key' name='$key' value='" . esc_attr($this->query[$key]) . "' /></div>" . "<div class='field'><span class='checkbox'><input type='checkbox' id='q_surname_ext' name='surname_ext' value='1'" . (! $this->surname_ext ? "" : " checked='checked'") . " /><label for='q_surname_ext'></label></span> <label for='q_surname_ext'>" . __('also search for similar surnames', WW1_TXTDOM) . "</label></div>" . "</div>" . "</div>\n";
                        break;
                    
                    case 'name':
                        // Текстовые поля
                        print "<div class='fieldset'>" . "<label for='q_$key'>$val:</label>" . "<div>" . "<div class='field'><input type='text' id='q_$key' name='$key' value='" . esc_attr($this->query[$key]) . "' /></div>" . "<div class='field'><span class='checkbox'><input type='checkbox' id='q_name_ext' name='name_ext' value='1'" . (! $this->surname_ext ? "" : " checked='checked'") . " /><label for='q_name_ext'></label></span> <label for='q_name_ext'>" . __('also search for abbreviations of names', WW1_TXTDOM) . "</label></div>" . "</div>" . "</div>\n";
                        break;
                    
                    case 'date':
                        // Поля дат
                        print "<div class='field'>" . "<label for='q_$key'>$val:</label>" . "<div><nobr>" . _x('from', 'Date from … to …', WW1_TXTDOM) . " <input type='date' id='q_$key' name='date_from' value='" . esc_attr($this->query['date_from']) . "' min='" . MIN_DATE . "' max='" . MAX_DATE . "' /></nobr> " . "<nobr>" . _x('to', 'Date from … to …', WW1_TXTDOM) . " <input type='date' name='date_to' value='" . esc_attr($this->query['date_to']) . "' min='" . MIN_DATE . "' max='" . MAX_DATE . "' /></nobr></div>" . "</div>\n";
                        break;
                    
                    default:
                        if (in_array($key, $this->dictionary_fields)) { // Списковые поля
                            print "<div class='field'>" . "<label for='q_$key'>$val:</label>" . "<div><select id='q_$key' name='${key}[]' multiple='multiple' size='5'>";
    ...
    function do_search()
    {
        ...
        foreach ($fields as $key) {
            ...
            if ($key == 'date_from') {
                ...
                // Числовые данные
                if (in_array($key, $this->ids_fields))
                    $key .= '_id'; // Проверка на поля с ID
                if (! is_array($val))
                ...
                if (is_array($val)) {
                    ...
                    $where[] = "p.`$key` IN (" . implode(', ', array_map(array(
                        gbdb(),
                        'data_escape'
                    ), $val)) . ')';
                } else {
                    ...
                    switch ($key) {
                        case 'surname':
                            $from_q = gbdb()->prepare_query('( SELECT k.person_id, MIN(k.surname_key_type)' . ' AS ktype FROM ?_idx_search_keys AS k WHERE ');
                            ...
                            if ($is_regex || ! $this->surname_ext) {
                                ...
                                $from_q .= '(k.surname_key_type = ' . GB_MK_SURNAME . ' AND (k.surname_key LIKE ' . implode(' OR k.surname_key LIKE ', $data) . ')) OR (k.surname_mask != "" AND ' . implode(' LIKE k.surname_mask OR ', $data2) . ' LIKE k.surname_mask) GROUP BY k.person_id ) AS isk';
                            } else {
                                ...
                                $data2 = gbdb()->data_escape($val_a, TRUE);
                                $from_q .= '(k.surname_key_type >= ' . GB_MK_SURNAME . ' AND k.surname_key_type < ' . (GB_MK_SURNAME + 100) . ' AND ' . gbdb()->prepare_query('k.surname_key IN (?keys)', array(
                                    'keys' => $metakeys
                                )) . ') OR (k.surname_mask != "" AND ' . implode(' LIKE k.surname_mask OR ', $data2) . ' LIKE k.surname_mask) GROUP BY k.person_id ) AS isk';
                                $q_fused_match = '(p.surname NOT LIKE ' . implode(' OR p.surname NOT LIKE ', gbdb()->data_escape(array_map(function ($text) {
                                    return "%$text%";
                        ...
                        case 'name':
                            $tmp = $is_regex || ! $this->name_ext ? GB_DBase::make_regex($val_a) : expand_names($val);
                            $where[] = 'UPPER(p.name) RLIKE ' . implode(' AND UPPER(p.name) RLIKE ', gbdb()->data_escape($tmp, TRUE));
                            break;
                        
                        case 'region':
                            $data = gbdb()->data_escape(GB_DBase::make_regex($val_a), TRUE);
                            $where[] = 'p.region_idx RLIKE ' . implode(' AND p.region_idx RLIKE ', $data);
                            break;
                        ...
                        case 'place':
                            ...
                            if ($this->query_mode == Q_SIMPLE) {
                                ...
                                $where[] = 'UPPER(p.place) RLIKE ' . implode(' AND UPPER(p.place) RLIKE ', $data);
                            }
                            break;
                    } // switch
                } // if
        ...
        $query = 'SELECT' . (! GB_DEBUG_SQL_PROF ? '' : ' SQL_NO_CACHE') . ' p.id' . $from . $where . $order;
        ...
        if (! empty($ids_part)) {
            ...
            $result = gbdb()->get_table('SELECT p.*, ' . $q_fused_match . ' AS fused_match FROM ?_v_persons AS p WHERE p.id IN (?ids)', array(
                'ids' => $ids_part
            ), 'id');
            //
            // Дополняем данные новыми (неформализуемыми) полями
            if (! empty($result)) {
                $add_fields = gbdb()->get_table('SELECT id, military_unit, place_of_event, estate_or_title,' . ' additional_info, birthdate FROM ?_persons_raw WHERE id IN (?ids)', array(
                    'ids' => $ids_part
                ), 'id');
