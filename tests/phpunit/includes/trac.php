...
class TracGitHubIssues
{
     ...
     *
     * @param string $repos_uri
     *            on GitHub ("User/Project")
     * @param int $issue_id            
     * @return bool|null if the issue is closed, FALSE if not closed, NULL on error
     */
    public static function isTicketClosed($repos_uri, $issue_id)
    {
        if (! isset(self::$trac_cache[$repos_uri])) {
            ...
            $file = DIR_TESTDATA . '/.trac-cache.' . str_replace(array(
                '/'
            ), array(
                '-'
            ), $repos_uri);
