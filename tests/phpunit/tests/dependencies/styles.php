...
class Tests_Dependencies_Styles extends GB_UnitTestCase {
	...
	public function test_protocols() {
		// Init
		$base_url_backup = _gb_styles()->base_url;
		_gb_styles()->base_url = 'http://example.com/genibase';
		$expected = '';
		$ver = get_siteinfo( 'version' );
		...
		$expected  .= "<link rel='stylesheet' id='reset-css-ftp-css' href='" . _gb_styles()->base_url . "ftp://yui.yahooapis.com/2.8.1/build/reset/reset-min.css?ver=$ver' type='text/css' media='all' />\n";
		...
		// Cleanup
		_gb_styles()->base_url = $base_url_backup;
	}
	...
	public function test_inline_styles_concat() {
		_gb_styles()->do_concat = true;
		_gb_styles()->default_dirs = array( '/gb-admin/', '/gb/css/' ); // Default dirs as in gb/script-loader.php
		...
		gb_print_styles();
		$this->assertEquals( $expected, _gb_styles()->print_html );
