 ...
 * @group testsuite
 */
class Tests_Basic extends GB_UnitTestCase
{
    ...
    // $readme = file_get_contents( BASE_DIR . '/readme.html' );
    // preg_match( '#<br /> Version (.*)#', $readme, $matches );
    // list( $version ) = explode( '-', $GLOBALS['gb_version'] );
    // $this->assertEquals( $version, trim( $matches[1] ), "readme.html's version needs to be updated to $version." );
    // }
    function test_license()
    {
        $license = file_get_contents(BASE_DIR . '/_license.txt');
        preg_match('#Copyright \(c\) 2014-(\d+) by #', $license, $matches);
        $this_year = date('Y');
        $this->assertEquals($this_year, $matches[1], "_license.txt's year needs to be updated to $this_year.");
    }
    ...
    function test_globals()
    {
        ...
        $this->assertEquals($test_foo, array(
            0 => 'foo',
            2 => 'baz'
        ));
    ...
    // <p>If a new user is created by GeniBase, the password will be set, by default, to "changeme". Quite suggestive, eh? ;)</p>
    // <ol id="authors"><form action="?import=wordpress&amp;step=2&amp;id=" method="post"><input type="hidden" name="_wpnonce" value="855ae98911" /><input type="hidden" name="_gb_http_referer" value="gb-test.php" /><li>Current author: <strong>Alex Shiels</strong><br />Create user <input type="text" value="Alex Shiels" name="user[]" maxlength="30"> <br /> or map to existing<select name="userselect[0]">
    // EOF;
    ...
    // <p>To make it easier for you to edit and save the imported posts and drafts, you may want to change the name of the author of the posts. For example, you may want to import all the entries as <code>admin</code>s entries.</p>
    // <p>If a new user is created by GeniBase, the password will be set, by default, to "changeme". Quite suggestive, eh? ;)</p>
    // <ol id="authors"><form action="?import=wordpress&amp;step=2&amp;id=" method="post"><input type="hidden" name="_wpnonce" value="***" /><input type="hidden" name="_gb_http_referer" value="gb-test.php" /><li>Current author: <strong>Alex Shiels</strong><br />Create user <input type="text" value="Alex Shiels" name="user[]" maxlength="30"> <br /> or map to existing<select name="userselect[0]">
    // EOF;
