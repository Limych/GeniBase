...
class Tests_Actions extends GB_UnitTestCase {
	...
	function test_current_action() {
		_gb_hooks()->current_filter[] = 'first';
		_gb_hooks()->current_filter[] = 'second'; // Let's say a second action was invoked.

		$this->assertEquals( 'second', current_action() );
	}

	function test_doing_filter() {
		_gb_hooks()->current_filter = array(); // Set to an empty array first
		...
		_gb_hooks()->current_filter[] = 'testing';
		...
		_gb_hooks()->current_filter = array();
	}
