...
class Tests_POMO_MO extends GB_UnitTestCase
{
    ...
    function test_mo_simple()
    {
        ...
        $this->assertEquals(array(
            'Project-Id-Version' => 'GeniBase 2.10-bleeding',
            'Report-Msgid-Bugs-To' => 'gb-polyglots@lists.automattic.com'
        ), $mo->headers);
        $this->assertEquals(2, count($mo->entries));
        $this->assertEquals(array(
            'dyado'
        ), $mo->entries['baba']->translations);
        $this->assertEquals(array(
            'yes'
        ), $mo->entries["kuku\nruku"]->translations);
    }
    ...
    function test_mo_context()
    {
        ...
        $this->assertEquals(2, count($mo->entries));
        $plural_entry = new Translation_Entry(array(
            'singular' => 'one dragon',
            'plural' => '%d dragons',
            'translations' => array(
                "oney dragoney",
        ...
        $single_entry = new Translation_Entry(array(
            'singular' => 'one dragon',
            'translations' => array(
                "oney dragoney"
            ),
    ...
    function test_translations_merge()
    {
        ...
        $guest->add_entry(new Translation_Entry(array(
            'singular' => 'green'
        )));
        $guest->add_entry(new Translation_Entry(array(
            'singular' => 'red'
        ...
        $this->assertEquals(array(), array_diff(array(
            'pink',
            'green',
            'red'
        ), array_keys($host->entries)));
    }
    ...
    function test_overloaded_mb_functions()
    {
        ...
        if ((ini_get("mbstring.func_overload") & 2) != 0) {
            ...
            $this->assertEquals(array(
                'Табло'
            ), $mo->entries['Dashboard']->translations);
        }
    }
