...
* Название(ия) места;
* Тип места;
* Ссылка на описание этого же места в проекте Викиданные (www.wikidata.org; или Википедия — см. ниже);
...
* Тип места указывается в окружении символов «%». Словарь всех типов см. ниже.  
Пример:  
`%губернский город%`
* Ссылка на Викиданные всегда начинается с символов «#wd:», после чего вплотную (без пробелов!) идёт идентификатор страницы в Викиданных. Чтобы увидеть этот объект, нужно перейти на сайт по адресу https://www.wikidata.org/wiki/<идентификатор>.  
Пример:  
`#wd:Q1190582` — означает адрес https://www.wikidata.org/wiki/Q1190582 (Бессарабская губерния)  
**Обратите внимание:** Допустимо вместо ссылки на Викиданные указывать ссылку на тот же объект на Википедии — при обработке машина автоматически заменит её на ссылку на Викиданные. Но, пожалуйста, не забудьте вплотную (без пробелов!) перед адресом указать символ «#» — это метка для машины, что дальше идёт ссылка на внешний ресурс.  
Пример:  
...
Примеры:  
`[+1812-05-16/+1917-12]` (с 16 мая 1812 по декабрь 1917),  
`[+1812/]` (с 1812 по настоящее время)
* Координаты пишутся после символа «@». Банально широта и долгота через запятую (без пробелов!). Часть координат есть в проекте Викиданные, часть придётся искать вручную. Рекомендую для этого пользовать старые карты (http://www.etomesto.ru/map-atlas_operationskarte-r/) — там слева вверху есть поле с надписями «Широта» и «Долгота». При клике на него открывается панель с разными вариантами написания координат того места, на которое указывает крестик в середине карты. Один из вариантов написания в точности подходит под наш формат — нужно только добавить перед ним символ «@».  
Пример:  
`@47.005555555,28.8575`
* Все поля выше можно указывать в любом порядке — при машинной обработке она сама всё корректно разберёт и после расставит в едином стандартном виде. Однако название места рекомендуется указывать всегда первым — так удобнее для восприятия. Отступы перед названием также нужны только для удобства людей — их можно опускать.  
Эти особенности полезны при редактировании. Например, можно не править предложения машины, а просто «в хвост» дописать новые данные.
...
Примеры:  
`?#wd:Q4130502` (возможная ссылка на Викиданные),  
...
### Словарь типов населённых пунктов

При указании типов населённых пунктов, пожалуйста, старайтесь придерживаться следующего набора вариантов (включая регистр букв). Допустимо придумывать свои варианты, но только если они существенно отличаются от приведённых тут.
...
1. В редакторе в меню выбираем пункт Синтаксисы > Задать свой синтаксис… (в английской версии меню: Language > Define your language…)
...
2. Разумно сначала проставить все ссылки на Викиданные на одном уровне (например, все уезды), потом прогнать файл через машину (для этого переслать его мне) — так она, возможно, отсеит заведомо неверные варианты предположений. Или найдёт новые;

3. Если предложенные машиной варианты с Викиданных не подходят, разумно воспользоваться поиском на том проекте (поле в верхнем правом углу любой страницы проекта) для определения нужной страницы, после чего идентификатор страницы перенести в наш файл;

4. В случае большого числа предложений от машины, обычно проще найти объект на Викиданных и посмотреть верный вариант идентификатора;
