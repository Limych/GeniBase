#!/bin/bash

echo "After Script"
echo "-- Repo Slug: $TRAVIS_REPO_SLUG"
echo "-- Repo Tag: $TRAVIS_TAG"
echo "-- PHP Version: $TRAVIS_PHP_VERSION"
echo "-- PULL Request: $TRAVIS_PULL_REQUEST"

if [ "$TRAVIS_REPO_SLUG" == "Limych/GeniBase" ] && [ "$TRAVIS_PULL_REQUEST" == "false" ] && [ "$TRAVIS_PHP_VERSION" == "5.6" ]; then

  echo "Generate"
  wget http://phpdox.de/releases/phpdox.phar
  php phpdox.phar

  echo "Publishing"
  # move docs to `home` directory
  cp -R docs/html $HOME/docs-latest

  cd $HOME
  git config --global user.email "travis@travis-ci.org"
  git config --global user.name "travis-ci"
  git config --global push.default simple
  git clone --quiet https://${GITHUB_TOKEN}@github.com/Limych/GeniBase-Docs > /dev/null

  cd GeniBase-Docs
  echo "-- Clean"
  git rm -rf ./ > /dev/null

  echo "-- Copy"
  cp -Rf $HOME/docs-latest/* ./

  echo "-- Push"
  git add -f .
  git commit -m "PHPDocumentor (Travis Build: $TRAVIS_BUILD_NUMBER@$TRAVIS_TAG)"
  git push -fq origin > /dev/null

  echo -e "Published to github.io\n"

fi
